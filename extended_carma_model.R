...
sim <- euler_maruyama_for_new_sde(p=3, q=2, as=c(-1,-2,-3), bs=c(1,2), cs=c(-3,-2,-1), Z=Z, X0=rep(0,3), S=1, T=500)
...
Z_values <-  runif(974, 0.0, 100)
Z2 <- stepfun(2:974, Z_values)

sim2 <- euler_maruyama_for_new_sde(p=2, q=1, as=c(-1,-1), bs=c(1), cs=c(-1), Z=Z2, X0=rep(0,2), S=1, T=974)
plot_new_sde(sim2)
